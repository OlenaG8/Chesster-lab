...
parser.add_argument("--rotate", type=int, default=0,
                    ...
                    help="Rotation of the board orientation relative to the camera (CW). "
                         "E.g., camera from the right = 90, from the back = 180, from the left = 270.")
...
def mouse_click(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        if len(points) < 4:
            ...
            print("[INFO] 4 points already selected. Press 'r' to reset or 's' to save.")

def remap_index(r_disp, c_disp, cam_rot):
    """Remap index (row, column) from the camera view to standard board notation."""
    if cam_rot == 0:
...
print("📸 Instructions:")
print(" 1️⃣ Point the camera to the position that WILL be used (e.g., from the side).")
print(" 2️⃣ Click the 4 corners of the board (order: top-left, top-right, bottom-right, bottom-left).")
print(" 3️⃣ Press 'r' to reset, 's' to save the sqdict.json result, 'q' to quit.")
...
while True:
    ...
    if len(points) == 4:
        ...
        # === Display standard board notation labels ===
        files = 'abcdefgh'
        ranks = '87654321'
    ...
    if key == ord('q'):
        ...
        if len(points) != 4:
            print("[WARN] You must click 4 points first before saving.")
            continue
